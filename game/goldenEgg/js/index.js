$(function () {
    ...
    function eggChange(i) {
        //抽奖
        let winData = winPrize(initPrizes);
        if (!winData) {
            return;
        }
        //砸蛋次数的变化
    ...
    $(function initData() {
        //初始化奖品列表
        let prizeSwiper = document.getElementById("swiper-wrapper");
        if (initPrizes && initPrizes.list.length > 0) {
            for (let i = 0; i < initPrizes.list.length; i++) {
                let prize=initPrizes.list[i];
                let str = `<li class="swiper-slide">
                        <img src="${prize.image}" alt="">
                        <p>${prize.name}</p>
                    </li>`
                prizeSwiper.innerHTML +=str;
            }
        }
        //初始化规则
        let activityCode=getQueryString("activityCode");
        let activity=getApiActivitys({"filters":`code=\"${activityCode}\"`})[0];
        let rule = document.getElementById("rule-text");
        rule.innerHTML=`<p>${activity.rule}</p>`
    })
